...
class NdpsLegalMapping(BaseModel):
    sections: List[SectionsCharged] = Field(
        description="List of NDPS sections applicable to the FIR. Return ONLY the top 5 most relevant and important sections."
    )
...
"""
    ...
    logger.info(f"Collected {len(all_sections_dict)} unique NDPS sections")
...
Retrieved NDPS Act Sections:
{sections_text}

Task:
Identify ONLY THE TOP 5 MOST RELEVANT AND IMPORTANT NDPS sections from the retrieved text above that are directly applicable to the FIR.

CRITICAL RULES:
1. Return EXACTLY 5 sections maximum - the most important and directly applicable ones.
2. Use ONLY the retrieved NDPS Act text above. Do not add external knowledge, legal interpretations, or assumptions.
...
6. Do NOT interpret or infer connections - use only what is explicitly stated in the retrieved NDPS Act text.
...
"""
    ...
    logger.info(f"Final mapping complete: {len(final_sections)} unique NDPS sections mapped")

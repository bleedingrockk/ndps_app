...
class BsaLegalMapping(BaseModel):
    sections: List[SectionsCharged] = Field(
        description="List of BSA sections applicable to the FIR. Return the most relevant sections.",
        max_items=5,
        min_items=1
    )
...
- Extract only BSA-related facts (evidence, confessions, statements, documents, witness accounts, seizures, digital evidence, forensic results).
- Each point must be a separate, clear, high-quality factual statement.
- Focus on facts that are most relevant for evidence and prosecution.
...
"""
    ...
    logger.info("Step 2: Querying RAG for relevant BSA sections (top 2 per point)")
    all_sections_dict = {}  # Use dict to deduplicate by section_number
    
    for idx, point in enumerate(points, 1):
        logger.debug(f"Querying RAG for point {idx}/{len(points)}")
        results = query_bsa(point, k=2)  # ← ONLY TOP 2
    ...
    final_prompt = f"""
You are an expert in BSA (Bharatiya Sakshya Adhiniyam) evidence law.
...
Task:
Identify the MOST RELEVANT AND IMPORTANT BSA sections from the retrieved text above that are directly applicable to the FIR.

CRITICAL RULES:
1. Return between 1-5 sections based on relevance (quality over quantity).
2. Use ONLY the retrieved BSA Act text above. Do not add external knowledge.
...
5. Prioritize sections that are most critical for evidence and proof.
...
8. Avoid duplicate or similar sections - each should cover a distinct evidentiary aspect.
9. If the same section appears multiple times in retrieved text, use it only ONCE with comprehensive description.

For each section, return:

- section_number: 
  Must match exactly as shown in retrieved text (e.g. "Section 20", "Section 20(1)", "Section 27")

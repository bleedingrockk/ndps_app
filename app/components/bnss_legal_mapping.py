...
class BnssLegalMapping(BaseModel):
    sections: List[SectionsCharged] = Field(
        description="List of BNSS sections applicable to the FIR. Return ONLY the top 5 most relevant and important sections."
    )
...
"""
    ...
    logger.info("Step 2: Querying RAG for relevant BNSS sections")
...
Task:
Identify ONLY THE TOP 5 MOST RELEVANT AND IMPORTANT BNSS sections from the retrieved text above that are directly applicable to the FIR.

CRITICAL RULES:
1. Return EXACTLY 5 sections maximum - the most important and directly applicable ones.
2. Use ONLY the retrieved BNSS Act text above. Do not add external knowledge, legal interpretations, or assumptions.
...
6. Do NOT interpret or infer connections - use only what is explicitly stated in the retrieved BNSS Act text.
